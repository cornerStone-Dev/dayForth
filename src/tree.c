...
static u32
tree_countr(Tree *tree, u32 count)
{
	if (tree == &nil || tree == 0) { return count; }
	count = tree_countr(tree->next[0], count);
	count += 1;
	return tree_countr(tree->next[1], count);
}
/*e*/u32
tree_count(Tree *tree)/*p;*/
{
	return tree_countr(tree, 0);
}
...
/*e*/Tree*
tree_insert(Tree **treep, u8 *key, u32 keyLen, void *value)/*p;*/
{
	Tree *tree = tree_add(treep, key, keyLen, value);
	if (tree != 0) { return tree; }
	return tree_find(*treep, key, keyLen);
}
